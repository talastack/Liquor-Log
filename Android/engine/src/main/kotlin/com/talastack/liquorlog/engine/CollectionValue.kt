package com.talastack.liquorlog.engine

/**
 * What the collection cost, for the people who want the number.
 *
 * Two findings pull in opposite directions and both are real.
 *
 * It is wanted, for recall and for insurance. And it causes genuine
 * distress: *"I added my collection to OnlyDrams when I had 50 bottles and
 * was taken aback by the value. Now I have at least double that number and
 * **I don't want to know**."* / *"Only thing I don't include is price --
 * guaranteed heart attack if I faced the reality of what I've spent."*
 *
 * So the number exists and is **off by default**. It is never shown as a
 * side effect of opening a screen; somebody has to ask for it.
 *
 * It is also strictly **what you paid**, never what a bottle is now worth.
 * Apps that ship their own secondary-market estimate get checked against
 * reality and lose. We do not have that data, we cannot keep it current, and
 * being wrong about it costs more than being silent.
 */
object CollectionValue {

    /** One bottle's contribution. */
    data class Holding(
        val purchasePriceCents: Int?,
        /** Killed bottles are excluded from the shelf total. See [Total]. */
        val isFinished: Boolean,
    )

    /** The total, with enough context that it cannot be read as more than it is. */
    data class Total(
        /** Sum of purchase prices for bottles still on the shelf. */
        val cents: Int,
        val bottlesCounted: Int,
        /**
         * Bottles on the shelf with no price recorded. These are why the
         * figure is a floor and not a total.
         */
        val bottlesWithoutPrice: Int,
    ) {
        /**
         * True when some bottles carry no price, which makes the sum a lower
         * bound rather than an answer.
         */
        val isPartial: Boolean get() = bottlesWithoutPrice > 0

        /** Never optional, and never omitted when the figure is partial. */
        val caveat: String
            get() {
                val base = "What you paid, not what it is worth."
                if (!isPartial) return base
                val bottles = if (bottlesWithoutPrice == 1) {
                    "1 bottle has"
                } else {
                    "$bottlesWithoutPrice bottles have"
                }
                return "$base $bottles no price recorded, so the real figure is higher."
            }
    }

    /** **Off until asked for.** The one place this default is written down. */
    const val SHOWN_BY_DEFAULT = false

    /**
     * Totals what is currently on the shelf.
     *
     * Finished bottles are excluded on purpose. The number people want is for
     * insurance and for recall -- both are about what is in the house.
     * Lifetime spend is the figure the quotes above are running away from,
     * and this app has no reason to compute it.
     */
    fun onTheShelf(holdings: List<Holding>): Total {
        val live = holdings.filter { !it.isFinished }
        val priced = live.mapNotNull { it.purchasePriceCents }
        return Total(
            cents = priced.sum(),
            bottlesCounted = priced.size,
            bottlesWithoutPrice = live.size - priced.size,
        )
    }
}
