 ...
 * lists, and [Plant.onTwoLists] says whether both carried it:
 *
...
object DistilleryPermit {
    ...
    data class Plant(
        ...
        val onTwoLists: Boolean,
        /** A closed plant, or one that only warehouses and bottles. */
        val note: String?
    )

    private val PERMIT = Regex("""DSP[\s\-]*([A-Z]{2})[\s\-]*([0-9]{1,6})""")

    /** "dsp ky 113", "DSP-KY-113", "DSP KY-113" -> "DSP-KY-113". */
    fun normalise(raw: String): String? {
        val upper = raw.uppercase()
        val match = PERMIT.find(upper) ?: return null
        val state = match.groupValues[1]
        val digits = match.groupValues[2].toIntOrNull() ?: 0
        return "DSP-$state-$digits"
    }

    fun lookup(raw: String): Plant? {
        val key = normalise(raw) ?: return null
        return plants.firstOrNull { it.number == key }
    }
     ...
     */
    fun find(text: String): String? = normalise(text)

    private fun plant(
        number: String,
        distillery: String,
        location: String,
        twoLists: Boolean = true,
        note: String? = null
    ) = Plant(
        number = number, distillery = distillery, location = location,
        onTwoLists = twoLists, note = note
    )
