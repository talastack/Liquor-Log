    ...
    public static func migrator() -> DatabaseMigrator {
        ...
        migrator.registerMigration("0001_init") { db in
            ...
            try db.create(table: "bottles") { t in
                ...
                t.column("topper_letter", .text)
                t.column("photo_file", .text)
            ...
            try db.create(table: "tastings") { t in
                ...
                t.column("source_note", .text)
                t.column("blind", .boolean).notNull().defaults(to: false)
                t.column("liked", .text)
